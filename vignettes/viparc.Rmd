...
See how functions `index()` and `fill_gaps()` can be used to do the job:

```{r}
dataset %>%
  mutate(amu_week = index(amu),
         disease2 = fill_gaps(disease),
         disease2_week = index(disease2)) %>% 
...
dataset %>%
  mutate(amu_week = index(amu),
         ...
         disease_week = ifelse(disease, index(disease), NA),
